...
(() => {
    ...
    // 保存当前账号ID
    async function saveCurrentAccount(siteId, accountId) {
        await ft({
            [`current_account_${siteId}`]: accountId
        })
    }
    // 获取当前账号ID
    async function getCurrentAccount(siteId) {
        let result = await ht([`current_account_${siteId}`]);
        return result[`current_account_${siteId}`] || null;
    }
...
\u5F53\u524D\u4FDD\u5B58\u7684\u8D26\u53F7\u6570\u636E\u5DF2\u8FBE\u5230\u6D4F\u89C8\u5668\u6269\u5C55\u5B58\u50A8\u4E0A\u9650\u3002\u8BF7\u5C1D\u8BD5\u5220\u9664\u4E00\u4E9B\u4E0D\u5E38\u7528\u7684\u8D26\u53F7\u5907\u4EFD\uFF0C\u6216\u7B49\u5F85\u6269\u5C55\u66F4\u65B0\u4EE5\u652F\u6301\u65E0\u9650\u5B58\u50A8\u6743\u9650\u3002`, "\u5B58\u50A8\u914D\u989D\u8D85\u9650");
    ...
    var P = class {
        ...
        start(t) {
            // Ad service disabled - do nothing
            console.log("[AdService] Ad service is disabled");
            return;
        }
        stop() {
        ...
        getCurrentAccount(t.site.id).then(currentAccountId => {
            y("account-list", !0), y("account-empty", !1), t.accounts.forEach(a => {
                ...
                if (a.id === currentAccountId) {
                    ...
                    badge.className = "current-account-badge";
                    badge.textContent = "当前";
                    badge.style.cssText = "background: #22c55e; color: white; font-size: 11px; padding: 2px 6px; border-radius: 4px; margin-left: 8px; font-weight: 600;";
                    h.appendChild(badge);
                }
                let S = document.createElement("div");
    ...
    async function Nt(n) {
        ...
        if (!e || e !== s) {
            ...
            // 保存当前账号ID
            await saveCurrentAccount(n.siteId, n.id);
            return
        }
        let c = await R({
    ...
    async function Rt() {
        ...
        let s = new Blob([JSON.stringify(data, null, 2)], {
                type: "application/json"
            }),
    ...
    async function Ot(n) {
        ...
        try {
            let e = JSON.parse(t);
            // 直接导入明文JSON，无需密码
            if (!e || e.version !== 1 || !Array.isArray(e.sites)) {
                throw new Error("备份文件格式不正确");
            }
            await b(e.sites);
            await d.alert(r("restoreSuccess"));
            window.location.reload();
        } catch (e) {
            let i = (e == null ? void 0 : e.message) || e;
            await d.alert(r("restoreFailed", [i]))
        }
    }
    async function Ft() {
        let n = await O();
